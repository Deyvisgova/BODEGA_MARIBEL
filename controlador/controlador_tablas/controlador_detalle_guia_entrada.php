...
if ($idGuiaEntrada <= 0 || $idProducto <= 0 || $cantidad <= 0 || $idLote <= 0 || $fechaVencimiento === '') {
...
$stmt = mysqli_prepare(
    $conn,
    'INSERT INTO guia_de_entrada_detalle (id_guia_entrada, id_producto, cantidad_entrada, id_lote, fecha_vencimiento) VALUES (?, ?, ?, ?, ?)'
);

if (!$stmt) {
    echo json_encode(['success' => false, 'message' => 'No se pudo preparar el registro del detalle.']);
    exit;
}

mysqli_stmt_bind_param($stmt, 'iiiis', $idGuiaEntrada, $idProducto, $cantidad, $idLote, $fechaVencimiento);
