 ...
 * siguen utilizando. Si el esquema no la tiene, se agrega como un campo opcional
 * para evitar el error de "Unknown column" que se estaba mostrando en guías de
 * salida al registrar el movimiento.
 */
function asegurarKardex(mysqli $conn): void
{
    $existeTabla = mysqli_query($conn, "SHOW TABLES LIKE 'kardex'");
    if (!$existeTabla || mysqli_num_rows($existeTabla) === 0) {
        mysqli_query($conn, "CREATE TABLE kardex (\n            id_kardex INT(11) NOT NULL AUTO_INCREMENT,\n            fecha DATE NOT NULL,\n            producto VARCHAR(200) NOT NULL,\n            tipo VARCHAR(20) NOT NULL,\n            descripcion VARCHAR(200) NOT NULL,\n            cantidad INT(11) NOT NULL,\n            saldo INT(11) NOT NULL,\n            referencia VARCHAR(50) NOT NULL,\n            kardex_encabezado_idkardex_encabezado INT(11) DEFAULT NULL,\n            PRIMARY KEY (id_kardex),\n            KEY idx_kardex_producto (producto)\n        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;");
        return;
    }

    $existeColumna = mysqli_query(
        $conn,
        "SHOW COLUMNS FROM kardex LIKE 'kardex_encabezado_idkardex_encabezado'"
    );

    if ($existeColumna && mysqli_num_rows($existeColumna) === 0) {
        mysqli_query(
            $conn,
            "ALTER TABLE kardex ADD COLUMN kardex_encabezado_idkardex_encabezado INT(11) DEFAULT NULL AFTER referencia"
        );
    }
}
...
function asegurarColumnaLoteContenedor(mysqli $conn): void
{
    ...
    if ($existeColumna && mysqli_num_rows($existeColumna) === 0) {
        mysqli_query($conn, "ALTER TABLE lote ADD COLUMN contenedor VARCHAR(100) NULL AFTER id_producto");
    }
}
...
switch ($accion) {
    case "btnAgregar":
        ...
        if ($domicilio_fiscal === '') {
            $domicilio_fiscal = 'Av. Los Próceres 145, Lima – San Juan de Lurigancho';
        }
        if ($punto_partida === '') {
            $punto_partida = 'Almacén Maribel– Jr. Huancavelica 234, Lima.';
        }
        if ($fecha_salida === '') {
