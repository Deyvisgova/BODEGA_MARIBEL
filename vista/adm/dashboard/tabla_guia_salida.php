...
if (!empty($producto)) {
    ...
    if (empty($detalleSalidaInicial) && $producto !== '') {
        $detalleSalidaInicial[] = [
            'producto' => $producto,
...
if (empty($detalleSalidaInicial)) {
    $detalleSalidaInicial[] = ['producto' => '', 'cantidad' => 1];
}
?>
<html lang="es">
<head>
...
<body id="page-top">
    ...
    <div class="container">
        <h3 class="text-center fw-bold">TABLA Guias de Salida</h3>
        <hr>

        <form action="../../../controlador/controlador_tablas/controlador_tabla_guia_salida.php" method="post" id="formGuiaSalida">
            <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="exampleModalLabel">Guía de salida</h1>
                        </div>
                        <div class="modal-body">
                            <div class="row g-3">
                                <div class="form-group col-md-4">
                                    <label>Id Guia de salida:</label>
                                    <input type="text" class="form-control" required name="id_guia_salida" id="id_guia_salida" value="<?php echo $id_guia_salida; ?>" readonly>
                                </div>

                                <div class="form-group col-md-4">
                                    <label>Fecha</label>
                                    <input type="date" class="form-control" required name="fecha_salida" id="fecha_salida" value="<?php echo $fecha_salida; ?>">
                                </div>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" class="form-control" required name="destino" id="destino" value="<?php echo $destino; ?>">
                                </div>

                                <div class="form-group col-md-6">
                                    <label for="">Encargado:</label>
                                    <input type="text" class="form-control" required name="encargado" id="encargado" value="<?php echo $encargado; ?>">
                                </div>
                                ...
                                <div class="form-group col-12">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div>
                                            ...
                                            <p class="text-muted small mb-0">Busca productos, define cantidades y descuéntalas al confirmar la entrega.</p>
                                        </div>
                                        <button type="button" class="btn btn-outline-secondary btn-sm" id="agregarFilaSalida">
                                            <i class="fa-solid fa-plus"></i> Añadir producto
                                        </button>
                                    </div>
                                    <div id="contenedorDetalleSalida" class="vstack gap-2 mt-2"></div>
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <input type="hidden" name="detalles" id="detalleSalida">
                            <input type="hidden" name="cantidad_salida" id="cantidadSalidaTotal" value="<?php echo $cantidad_salida; ?>">
                            <input type="hidden" name="producto" id="productosSalidaResumen" value="<?php echo htmlspecialchars($producto, ENT_QUOTES, 'UTF-8'); ?>">
            ...
            <div class="row mb-3">
                ...
                <div class="col-12 col-sm-9 d-flex justify-content-sm-end mb-2">
                    <a href="pdfs/pdf_guia_salida.php" target="_blank" class="btn btn-danger btn-sm shadow-sm">
    ...
    <script>
        const detalleSalidaInicial = <?php echo json_encode($detalleSalidaInicial); ?>;

        const contenedorSalida = document.getElementById('contenedorDetalleSalida');
        const detalleSalidaInput = document.getElementById('detalleSalida');
        const cantidadSalidaTotal = document.getElementById('cantidadSalidaTotal');
        const productosSalidaResumen = document.getElementById('productosSalidaResumen');
        const btnAgregarFilaSalida = document.getElementById('agregarFilaSalida');
        const formSalida = document.getElementById('formGuiaSalida');

        function crearFilaSalida(detalle = {producto: '', cantidad: 1}) {
            const fila = document.createElement('div');
            fila.classList.add('row', 'g-2', 'align-items-center', 'detalle-item');
            fila.innerHTML = `
                <div class="col-md-7">
                    <input list="listaProductosSalida" class="form-control input-producto" placeholder="Producto" value="${detalle.producto}" required>
                </div>
                <div class="col-md-3">
            ...
            fila.querySelector('.btn-remover').addEventListener('click', () => {
                fila.remove();
                actualizarDetallesSalida();
            });

            fila.querySelector('.input-producto').addEventListener('input', actualizarDetallesSalida);
            fila.querySelector('.input-cantidad').addEventListener('input', actualizarDetallesSalida);

            contenedorSalida.appendChild(fila);
        }

        function actualizarDetallesSalida() {
            const filas = contenedorSalida.querySelectorAll('.detalle-item');
            const detalles = [];
            let total = 0;
            let resumen = [];
            ...
            detalleSalidaInput.value = JSON.stringify(detalles);
            cantidadSalidaTotal.value = total;
            productosSalidaResumen.value = resumen.join(', ');
        }

        btnAgregarFilaSalida.addEventListener('click', () => {
            crearFilaSalida();
        });

        formSalida.addEventListener('submit', (event) => {
            actualizarDetallesSalida();
            if (!detalleSalidaInput.value || detalleSalidaInput.value === '[]') {
                event.preventDefault();
                alert('Agrega al menos un producto a la guía.');
            }
        });

        detalleSalidaInicial.forEach(item => crearFilaSalida(item));
        actualizarDetallesSalida();
    </script>
